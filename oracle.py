...
# Update specific table by name
# python oracle.py --tasks allusers

# Update multiple tables
# python oracle.py --tasks allusers,allsanim,allpayment

# Update by number
# python oracle.py --tasks 1,2,9

# Mix names and numbers
# python oracle.py --tasks allusers,2,allpayment
...
def input_info():
    parser = argparse.ArgumentParser(description='Oracle Data Sync Script')
    parser.add_argument('--tasks', type=str,
                        help='Comma-separated list of tasks to run. Can be table names (e.g., allsanim,allusers) or numbers (1-12), or "all" for all tables. Default: all',
                        default='all')
    parser.add_argument('--RUNALL', type=str,
                        help='Run all tasks immediately without scheduling. Options: true/false. Default: true',
                        default='true')
    args = parser.parse_args()
    selected_tasks = args.tasks.split(',')
    RUNALL = args.RUNALL.lower()

    # Convert table names to task numbers for backward compatibility
    table_to_number = {
        'allusers': '1',
        'allsanim': '2',
        'allbakhshodegi': '3',
    ...
    # Convert table names to numbers if provided
    converted_tasks = []
    for task in selected_tasks:
        task = task.strip().lower()
        if task in table_to_number:
            converted_tasks.append(table_to_number[task])
        else:
            converted_tasks.append(task)

    return converted_tasks, RUNALL
...
params = {
    ...
    'allusers': [excel_files['allusers'], get_sql_sanimusers(tblname=tbl_names['allusers'], ostan_txt='_khozestan'), tbl_names['allusers']],
...
def run_specific_tables(table_keys):
    """Run specific tables with proper batching and multi-threading"""
    df_tax_types = get_tax_types()
    for key in table_keys:
        if key not in tbl_names:
            logging.warning(f"Unknown table key: {key}")
            continue

        value = tbl_names[key]
        logging.info(f"Processing table: {key} ({value})")
        ...
        # Count rows and create buckets
        num_threads = 0
        while (num_threads <= 0):
            buckets, num_threads, count = count_tbls(tbl=value)

        logging.info(
            f"Table {key} has {count} rows, using {num_threads} threads with {len(buckets)} buckets")

        # Execute multi-threaded fetch
        executor = ThreadPoolExecutor(num_threads)
        jobs = [executor.submit(get_allSanim, excel_files[key], get_sql_sanimusers(
            item[0], item[1], value, ostan_txt), value, False, False, df_tax_types) for item in buckets]
        wait(jobs)
...
def schedule_tasks(time_to_run='15:30'):
    # Map task numbers to table keys
    task_to_table = {
        '1': 'allusers',
        '2': 'allsanim',
        '3': 'allbakhshodegi',
    ...
    if 'all' in tasks:
        ...
        tables_to_process = []
        for task in tasks:
            if task in task_to_table:
                tables_to_process.append(task_to_table[task])
            elif task.lower() in tbl_names:
                tables_to_process.append(task.lower())

        if tables_to_process:
            schedule.every().day.at(time_to_run).do(run_specific_tables, tables_to_process)
