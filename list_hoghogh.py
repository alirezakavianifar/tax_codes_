...
path_listhoghogh = os.path.join(project_root, r'saved_dir\listhoghogh')
log_path = os.path.join(path_listhoghogh, 'soratmoamelat.txt')

maybe_make_dir([path_listhoghogh])
...
def run_it(*args, **kwargs):
    x = Scrape()
    if 'field' in kwargs:
        x.scrape_list_hoghogh(path=path_listhoghogh,
                              headless=HEADLESS,
                              creds=CREDS,
                              field=kwargs['field'])
    else:
